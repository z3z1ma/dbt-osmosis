...
{% macro _dbt_osmosis_compare_queries(a_query, b_query, primary_key=None) -%}
  {{ return(adapter.dispatch('_dbt_osmosis_compare_queries')(a_query, b_query, primary_key)) }}
{%- endmacro %}
...
{% macro _dbt_osmosis_compare_relations(a_relation, b_relation, exclude_columns=[], primary_key=None) %}
...
{% macro _dbt_osmosis_compare_relation_columns(a_relation, b_relation) %}
  {{ return(adapter.dispatch('_dbt_osmosis_compare_relation_columns')(a_relation, b_relation)) }}
{% endmacro %}

{% macro default___dbt_osmosis_compare_relation_columns(a_relation, b_relation) %}

with a_cols as (
    {{ get_columns_in_relation_sql_dosmo(a_relation) }}
),

b_cols as (
    {{ get_columns_in_relation_sql_dosmo(b_relation) }}
)
...
{% macro _dbt_osmosis_compare_column_values(a_query, b_query, primary_key, column_to_compare) -%}
  {{ return(adapter.dispatch('_dbt_osmosis_compare_column_values')(a_query, b_query, primary_key, column_to_compare)) }}
{%- endmacro %}

{% macro default___dbt_osmosis_compare_column_values(a_query, b_query, primary_key, column_to_compare) -%}
with a_query as (
    {{ a_query }}
),
...
def inject_macros(dbt: DbtOsmosis) -> None:
    ...
    macro_overrides = {}
    for node in dbt.macro_parser.parse_remote(AUDIT_MACROS):
