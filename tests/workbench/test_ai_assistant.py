...
def test_render_shows_truthful_unavailable_panel_without_top_level_state(monkeypatch):
    ...
    assert any("not wired into the real documentation pipeline" in text for text in fake_mui.typography_calls)
    assert any("does not generate or accept suggestions" in text or "misleading to generate or accept suggestions" in text for text in fake_mui.typography_calls)
